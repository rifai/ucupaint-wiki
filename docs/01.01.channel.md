...
Each channel has at least one input and one output, but the Normal channel can have multiple inputs and outputs (when [Displacement Setup](#displacement-setup) is enabled).
